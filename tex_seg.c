...
int main(int argc, char *argv[]) {
  ...
  vocab = (char *)malloc((long long)200 * max_w * sizeof(char));     

  M = (float *)malloc((long long)200 * (long long)size * sizeof(float));
  if (M == NULL) {
  ...
  V = (char *)malloc((long long)lines * max_w * sizeof(char) + (long long)lines * (long long)size * sizeof(float));
printf("sentence65\n");
  ...
  for(b=0;b<lines;b++){
	  fscanf(inp,"%s",&V[b*max_w*sizeof(char)+(long long)b*(long long)size*sizeof(float)]);
  }

  for(b=0;b<lines;b++){
	  printf("%s\n",&V[b*max_w*sizeof(char)+(long long)b*(long long)size*sizeof(float)]);
  }
...
/*for(b=0;b<lines;b++){
	printf("%s\n",&vocab[0*max_w*sizeof(char)]);
}
*/
b =0;
while(&V[b*max_w*sizeof(char)+(long long)b*(long long)size*sizeof(float)] != EOF){
	fprintf(outp,"%s\n",&V[b*max_w*sizeof(char)+(long long)b*(long long)size*sizeof(float)]);
	b++;
}
